...
export class InnerRequest {
  ...
  static withBody(_body: string): InnerRequest {
    let userId = CookieService.get('user-id');
  ...
  static fromString(response: string): InnerRequest {
    const temp = new InnerRequest();
    ...
    if (temp.getH('user-id') !== CookieService.get('user-id'))
      CookieService.set('user-id', temp.getH('user-id'));
    return temp;
  }
